...
const DownloadButton: React.FC<{ imageUrl: string; index: number }> = ({ imageUrl, index }) => {
    const handleDownload = () => {
        ...
        link.href = imageUrl;
        link.download = `tiktok-affiliate-image-${index + 1}.jpeg`;
...
export const ImageGrid: React.FC<ImageGridProps> = ({ images }) => {
  return (
    <div className="text-center">
        <h2 className="text-3xl font-bold mb-8 text-purple-300">Your Generated Images</h2>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
            {images.map((imgSrc, index) => (
                <div key={index} className="relative aspect-[9/16] rounded-lg overflow-hidden shadow-lg group border-2 border-gray-700">
                    <img src={imgSrc} alt={`Generated content ${index + 1}`} className="w-full h-full object-cover" />
